...
async function bundle(options?: BuildOptions) {
  await build({
    outfile: `${cwd()}/dist/es/vue-dlib.esm.js`,
    bundle: true,
    entryPoints: [
      `${cwd()}/src/packages/vue-dlib.ts`,
    ],
    plugins: [vue()],
    loader: { '.png': 'dataurl' },
    external: ['vue', 'vue-dlib/*', '@vue/*'],
    format: 'esm',
    minify: true,
    ...options,
  })
...
async function combineCss() {
  ...
  await Promise.all([
    fs.promises.writeFile(
      `${cwd()}/dist/es/vue-dlib.esm.css`,
      content
    ),
    fs.promises.writeFile(
      `${cwd()}/dist/lib/vue-dlib.umd.css`,
      content
    ),
  ])

  const name = 'vue-dlib.min.css'
  await Promise.all([
    fs.promises.rename(
      `${cwd()}/dist/es/vue-dlib.esm.css`,
      `${cwd()}/dist/es/${name}`
